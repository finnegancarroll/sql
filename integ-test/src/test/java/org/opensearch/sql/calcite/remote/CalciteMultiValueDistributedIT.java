...
/**
 * Multi-shard (distributed) integration tests for multi-value keyword fields, driven through the SQL
 * plugin PPL endpoint against the analytics engine. Complements the single-shard
 * {@link CalciteMultiValueKeywordOperatorIT} by exercising the coordinator reduce / cross-shard FFI
 * paths that a single shard never triggers:
 *
 * <ul>
 *   <li>Two-phase (PARTIAL on shard, FINAL on coordinator) aggregation over a LIST group key.
 *   <li>Backend routing assertions via the profile API (AE {@code datafusion} vs Lucene delegation).
 ...
 * </ul>
 *
 * <p>Assertions are order-independent where result ordering is not query-guaranteed (rows are sorted
 * before comparison) and exact otherwise.
 */
public class CalciteMultiValueDistributedIT extends OpenSearchRestTestCase {
  ...
  public void testMultiShardPartialAggProfileHasPartialAndFinalStages() throws IOException {
    ...
    JSONObject result =
        executeWithProfile(
            "source = " + INDEX + " | mvexpand tags | stats count() as c by tags", "/_plugins/_ppl");
  ...
  public void testMultiValueGroupByRoutesToAnalyticsEngine() throws IOException {
    ...
    JSONArray fullPlan = result.getJSONObject("profile").getJSONObject("plan").getJSONArray("full_plan");
  ...
  public void testMultiShardListProjection() throws IOException {
    ...
    // boundary. Each doc keeps its array intact; assert the full (id, tags) set order-independently.
  ...
  private JSONObject executeWithProfile(String ppl, String endpoint) throws IOException {
    ...
    request.setJsonEntity(String.format(Locale.ROOT, "{\"query\": \"%s\", \"profile\": true}", ppl));
    return new JSONObject(entityAsString(client().performRequest(request)));
  }
