...
public class OpenSearchDataType implements ExprType, Serializable {
        ...
        (k, v) -> {
          ...
          OpenSearchDataType fieldType =
              OpenSearchDataType.of(
                  ...
                  innerMap);
          // A field mapped `multi_value: true` (e.g. keyword promoted to multi-value) is stored as a
          // LIST column and must surface as ARRAY so explicit array operators (array_contains,
          // array_length, mvjoin, ...) type-check against it. Wrap the element type as ARRAY.
   ...
   */
  /**
   * Builds an ARRAY-typed OpenSearchDataType wrapping the given element type. Used for
   * {@code multi_value} fields so array operators type-check against a Calcite ARRAY.
   */
  public static OpenSearchDataType ofArray(OpenSearchDataType elementType) {
    ...
    // sufficient for array operators to bind. elementType retained as a param for future element typing.
    return OpenSearchDataType.of(org.opensearch.sql.data.type.ExprCoreType.ARRAY);
  }
